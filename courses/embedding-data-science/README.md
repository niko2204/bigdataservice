# 임베딩 중심 LLM 데이터 과학 — 한국어 10주 강의자료
...
> **텍스트·문서·이미지 같은 비정형 데이터를 벡터로 표현하고, 분류·검색·RAG·멀티모달
> 문제에 적용한 뒤 데이터와 평가 지표로 성능을 검증하는 임베딩 중심 LLM 데이터 과학**

이 강의는 특정 LLM의 사용법이나 프롬프트 작성법만 다루지 않습니다. TF-IDF와 작은 행렬을
기준선으로 삼아 임베딩이 무엇을 보존하고 잃는지 이해하고, 모델의 결과를 정량 지표와 실패
사례로 분석하여 재현 가능한 데이터 서비스를 설계하는 데 초점을 둡니다.

### 10주 학습 흐름

1. **표현의 기초(1–3주):** 벡터·유사도, 토큰화·TF-IDF, 단어 임베딩을 학습합니다.
2. **문맥 표현(4–5주):** Transformer의 Attention과 문장·문서 임베딩을 다룹니다.
3. **분석과 평가(6–7주):** 분류·군집·시각화와 P@K·R@K·MRR·nDCG를 이용해
   임베딩의 품질을 검증합니다.
4. **서비스 응용(8–9주):** 벡터 검색·근거 기반 RAG와 텍스트–이미지 멀티모달 검색을
   구현합니다.
5. **통합(10주):** 기준선과 임베딩 모델을 비교하고 오류를 설명하는 작은 데이터 서비스를
   완성합니다.

### 학습 후 할 수 있는 일

- 희소 표현과 밀집 임베딩의 차이를 수학적·실험적으로 설명할 수 있습니다.
- 문제에 맞는 유사도와 검색 평가 지표를 선택하고 작은 예제를 손으로 계산할 수 있습니다.
- 임베딩을 이용한 분류·군집·검색·RAG·멀티모달 실험을 Notebook으로 재현할 수 있습니다.
- 평균 점수뿐 아니라 질의별 실패, 편향, 검색 오류와 생성 오류를 구분해 분석할 수 있습니다.
- 기준선, 개선 실험, 모델 카드와 근거를 갖춘 소규모 임베딩 데이터 서비스를 설계할 수 있습니다.
